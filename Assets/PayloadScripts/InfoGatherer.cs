    ...
    public class InfoGatherer : MonoBehaviour
    {
        ...
        void Initialize()
        {
            ...
            if (saveMinMaxAvgData)
            {
                ...
                {
                    totalFPS = temp;
                    dataAmount = PlayerPrefs.GetInt("FPSDataAmount");
                }
            }
            else
            {
        ...
        public void SetEnabled(InfoType type, bool enabled, Text text = null)
        {
            switch (type)
            {
                case InfoType.FPS:
                    if (enabled)
                    {
                        EnableInfo(UpdateFPS, (int) type, text);
                        fpsInitialColor = text == null ? texts[0].color : text.color;
                    }
                    else DisableInfo(UpdateFPS);
                    break;
                case InfoType.FPSMin:
                    if (enabled) EnableInfo(UpdateFPSMin, (int) type, text);
                    else DisableInfo(UpdateFPSMin);
                    break;
                case InfoType.FPSMax:
                    if (enabled) EnableInfo(UpdateFPSMax, (int) type, text);
                    else DisableInfo(UpdateFPSMax);
                    break;
                case InfoType.FPSAvg:
                    if (enabled) EnableInfo(UpdateFPSAvg, (int) type, text);
                    else DisableInfo(UpdateFPSAvg);
                    break;
                case InfoType.GPU:
                    if (enabled) EnableInfo(UpdateGPU, (int) type, text);
                    else DisableInfo(UpdateGPU);
                    break;
                case InfoType.GPUMemory:
                    if (enabled) EnableInfo(UpdateGPUMem, (int) type, text);
                    else DisableInfo(UpdateGPUMem);
                    break;
                case InfoType.CPU:
                    if (enabled) EnableInfo(UpdateCPU, (int) type, text);
                    else DisableInfo(UpdateCPU);
                    break;
                case InfoType.CPUCores:
                    if (enabled) EnableInfo(UpdateCPUCores, (int) type, text);
                    else DisableInfo(UpdateCPUCores);
                    break;
                case InfoType.CPUFrequency:
                    if (enabled) EnableInfo(UpdateCPUFreq, (int) type, text);
                    else DisableInfo(UpdateCPUFreq);
                    break;
                case InfoType.RAMTotal:
                    if (enabled) EnableInfo(UpdateRAMTotal, (int) type, text);
                    else DisableInfo(UpdateRAMTotal);
                    break;
                case InfoType.RAMAlloc:
                    if (enabled) EnableInfo(UpdateRAMAlloc, (int) type, text);
                    else DisableInfo(UpdateRAMAlloc);
                    break;
                case InfoType.OperatingSystem:
                    if (enabled) EnableInfo(UpdateOS, (int) type, text);
                    else DisableInfo(UpdateOS);
                    break;
            }
        }
        ...
        void EnableInfo(Action a, int i, Text text)
        {
            if (!actions.Contains(a))
            {
                if (!texts[i] && !text)
                {
                    Debug.LogError("Stats&Info  -  You are trying to enable the info " + (InfoType) i +
                                   ", but its text field is empty. Either set it in the inspector, or pass a 'Text' value with the 'SetEnabled' function.");
                    return;
                }
                actions.Add(a);
        ...
        {
            texts[10].text = (Profiler.GetTotalReservedMemoryLong() / 1000000f).ToString("0") + " MB";
        }
